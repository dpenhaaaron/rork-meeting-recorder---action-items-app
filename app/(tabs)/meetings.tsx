...
export default function MeetingsScreen() {
  ...
  const shareViaSMS = async (meeting: Meeting) => {
    if (!meeting.artifacts) {
      Alert.alert('No Data', 'Meeting has not been processed yet.');
      return;
    }
    ...
    try {
      let message = `Meeting Summary: ${meeting.title}\n`;
      message += `${new Date(meeting.date).toLocaleDateString()}\n\n`;
      
      if (meeting.artifacts.summaries?.executive_120w) {
        message += meeting.artifacts.summaries.executive_120w.substring(0, 100) + '...\n\n';
      }
      
      if (meeting.artifacts.action_items && meeting.artifacts.action_items.length > 0) {
        message += `Action Items (${meeting.artifacts.action_items.length}):\n`;
        meeting.artifacts.action_items.slice(0, 3).forEach((item, index) => {
          message += `• ${item.title}\n`;
        });
        if (meeting.artifacts.action_items.length > 3) {
          message += `...and ${meeting.artifacts.action_items.length - 3} more\n`;
        }
      }
      ...
      const messagingOptions = [
        ...
        { name: 'WhatsApp', url: `whatsapp://send?text=${encodeURIComponent(message)}` },
        { name: 'Telegram', url: `tg://msg?text=${encodeURIComponent(message)}` },
      ];
      
      // Check which apps are available
      const availableApps = [];
      for (const app of messagingOptions) {
        try {
      ...
      if (availableApps.length === 0) {
        Alert.alert('No Messaging Apps', 'No messaging apps are available on this device.');
        return;
      }
      ...
      if (availableApps.length === 1) {
        ...
        Alert.alert(
          ...
          'Select how you want to share this meeting summary:',
          [
